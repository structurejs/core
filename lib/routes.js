...
function getActionByVerb(controllerName) {

  return function(req, res, next) {

    req.params.controller = controllerName
    getControllerActionByVerb(req, res, next)
...
function routes(server) {
  ...
  server.all('/api/v0.1/apps/:sid',          getActionByVerb('apps'))
  server.all('/api/v0.1/organizations/:sid', getActionByVerb('organizations'))
  server.all('/api/v0.1/templates/:sid',     getActionByVerb('templates'))
